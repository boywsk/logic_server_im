...
#
#pid file
PIDFILE=/var/run/logic-server.pid
###################################
#环境变量及程序执行参数
...
APP_HOME=/data/logic-server

#需要启动的Java主程序（main方法类）
APP_MAINCLASS=cn.com.gome.logic.Main
...
JAVA_OPTS="-Xms1024m -Xmx2048m -Xmn512m -Djava.awt.headless=true -XX:MaxPermSize=512m"
...
checkpid() {
   ...
   #fi
	if test -e $PIDFILE; then
		pid=`cat $PIDFILE`
		#echo "================================"
		echo "$PIDFILE already exist! (pid=$pid)"
		#echo "================================"
		#ps -p $pid > /dev/null
		#echo $?
		if ps -p $pid > /dev/null; then
			psid=$pid
			#echo "the app process exist!"
			#if kill -9 $pid;then

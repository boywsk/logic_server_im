...
public class TCPProtocolDecoder extends ByteToMessageDecoder {
	...
	protected void decode(ChannelHandlerContext ctx, ByteBuf buf, List<Object> out) throws Exception {
		try {
			...
			// 判断包头长度
			if (buf.readableBytes() < (Global.PACK_HEAD_LENGTH + 1)) {// 不够包头长度65
			...
			if (startTag != Global.HEAD_START_TAG) {
				log.error("[decode] error,msg is Illegal");
				ctx.close();
				return;
			}
			...
			log.info("[decode] head size=[{}],cmd=[{}]", head.array().length, cmd);
			// 计算有效数据长度
			...
			log.info("[decode] head size=[{}],body size=[{}]", head.array().length, protoBody.array().length);
